...
func main() {
	var p printer
	p = user{username: "Test", id: 1}
	fmt.Println(p.Print())

	p = menuItem{name: "Coffee", prices: map[string]float64{"small": 1.65}}
	fmt.Println(p.Print())

	// panics because the p is currently holding a menuItem (line 42)
	u, ok := p.(user)
	fmt.Println(u, ok)

	// succeeds because p is a menuItem
	mi, ok := p.(menuItem)
	fmt.Println(mi, ok)
